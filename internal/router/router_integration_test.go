...
func TestRouterIntegration(t *testing.T) {
	...
	cfg := &config.Config{
		Redis: config.RedisConfig{
			URL:      "redis://localhost:6379/15", // Use test DB
			Password: "",
			DB:       15,
		},
	...
	modelManager := models.NewModelManager(logger)
	
	// Register test models for load balancing tests
	err := modelManager.RegisterModel("gpt-4", "openai", map[string]interface{}{
		"api_key": "test-key-1",
	})
	require.NoError(t, err)
	
	err = modelManager.RegisterModel("gpt-3.5-turbo", "openai", map[string]interface{}{
		"api_key": "test-key-2",
	})
	require.NoError(t, err)

	// Setup pricing manager
	pricingManager := config.NewModelPricingManager()
...
func testLoadBalancing(t *testing.T, router http.Handler) {
	...
	t.Run("Model Selection", func(t *testing.T) {
		...
		// Should get 503 (service unavailable) because no real providers
		// but this proves routing and load balancing logic works
...
func TestRouterLatencyRequirements(t *testing.T) {
	...
	cfg := &config.Config{
		Redis: config.RedisConfig{URL: "redis://localhost:6379/15"},
...
func TestRouterFailover(t *testing.T) {
	...
	// Register multiple models for failover testing
	err := modelManager.RegisterModel("primary-model", "openai", map[string]interface{}{
		"api_key": "primary-key",
	})
	require.NoError(t, err)

	err = modelManager.RegisterModel("backup-model", "openai", map[string]interface{}{
		"api_key": "backup-key",
	})
	require.NoError(t, err)
